...
def pub_birdviewCam1_worker():
    bridge = CvBridge()
    pubcam1 = rospy.Publisher('~birdviewCam1_camera', Image, queue_size=10)
    time.sleep(0.51)
    rear_UI_h = 500
    rear_UI_w = 500
    rospy.loginfo("BirdviewCam1 vision bird publishing ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        bird_t1 = undistort(bird_1, K, D, DIM, debug_imshow)
        combineImg1 = cv2.rotate(bird_t1, cv2.ROTATE_90_CLOCKWISE)
        imgBirdview1 = cv2.resize(combineImg1, (500, 990))
        image_message = bridge.cv2_to_imgmsg(imgBirdview1, "bgr8")
        image_message.header = Header(stamp=rospy.Time.now())
        image_message.header.frame_id = 'birdviewcam1'
        pubcam1.publish(image_message)
        rate.sleep()

def pub_birdview_worker():
    bridge = CvBridge()
    pub = rospy.Publisher('~birdview_camera', Image, queue_size=10)
    time.sleep(0.9)
    rear_UI_h = 500
    rear_UI_w = 500
    rospy.loginfo("Birdview vision bird publishing ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        """
        if is_HD_Image:
            bird_4und = undistort(bird_4, K_HD, D_HD, DIM_HD, debug_imshow)
        else:
            bird_4und = undistort(bird_4, K, D, DIM, debug_imshow)
        bird_2mod = bird_2[int(oriImg_h * cam2trim_h_s):int(oriImg_h * cam2trim_h_e),
                 int(oriImg_w * cam2trim_w_s):int(oriImg_w * cam2trim_w_e)]
        bird_3mod = bird_3[int(oriImg_h * cam3trim_h_s):int(oriImg_h * cam3trim_h_e),
                 int(oriImg_w * cam3trim_w_s):int(oriImg_w * cam3trim_w_e)]
        bird_1_rot = bird_1
        bird_2_rot = cv2.rotate(bird_2mod, cv2.ROTATE_90_COUNTERCLOCKWISE)
        bird_3_rot = cv2.rotate(bird_3mod, cv2.ROTATE_90_COUNTERCLOCKWISE)
        if is_HD_Image:
            bird_4_rot = cv2.resize(bird_4und, (640, 360))
        else:
            bird_4_rot = bird_4und
        combineImg = cv2.hconcat([bird_2_rot, bird_3_rot])
        #combineImg = cv2.vconcat([bird_1_rot, combineImg, bird_4_rot])
        combineImg = cv2.vconcat([combineImg, bird_4_rot])
        """
        bird_t = undistort(bird_2, K, D, DIM, debug_imshow)
        combineImg = cv2.rotate(bird_t, cv2.ROTATE_90_CLOCKWISE)
...
def pub_birdr5_worker():
    ...
    rospy.loginfo("Birdview vision r5 publishing ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        if is_HD_Image:
            bird_4und = undistort(bird_4, K_HD, D_HD, DIM_HD, debug_imshow)
            ...
            bird_4und = undistort(bird_4, K, D, DIM, debug_imshow)
        frameBTF = cv2.warpPerspective(bird_4und, M5, (500, 500))
        # draw_line(frameBTF)
...
def pub_birdr10_worker():
    ...
    pts_d = np.float32([[0, 0], [rear_UI_w, 0], [0, rear_UI_h], [rear_UI_w, rear_UI_h]])
    M10 = cv2.getPerspectiveTransform(pts_o, pts_d)
    rospy.loginfo("Birdview vision r10 publishing ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        if is_HD_Image:
            bird_4und = undistort(bird_4, K_HD, D_HD, DIM_HD, debug_imshow)
            ...
            bird_4und = undistort(bird_4, K, D, DIM, debug_imshow)
        frameBTF = cv2.warpPerspective(bird_4und, M10, (500, 500))
        # draw_line(frameBTF)
        image_message = bridge.cv2_to_imgmsg(frameBTF, "bgr8")
        image_message.header = Header(stamp=rospy.Time.now())
...
def pub_birdr20_worker():
    ...
    rospy.loginfo("Birdview vision r20 publishing ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        # For Cali??????????????????
        ...
        if is_HD_Image:
            ...
            bird_4und = undistort(bird_4, K, D, DIM, debug_imshow)
        frameBTF = cv2.warpPerspective(bird_4und, M20, (500, 500))
        # draw_line(frameBTF)
        image_message = bridge.cv2_to_imgmsg(frameBTF, "bgr8")
        image_message.header = Header(stamp=rospy.Time.now())
...
def show_testimg():
    global bird_2
    bird_cap = cv2.VideoCapture(bird_2_addr)
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        temp, bird_t = bird_cap.read()
        bird_t = cv2.resize(bird_t, (640, 360))
        bird_t = undistort(bird_t, K, D, DIM, debug_imshow)
        bird_t = cv2.rotate(bird_t, cv2.ROTATE_90_CLOCKWISE)
        cv2.imshow('bird_t', bird_t)
        rate.sleep()
...
def get_cam1_worker():
    ...
    rospy.loginfo("Birdview vision cam1 ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        temp, bird_t = bird_1_cap.read() #1
        bird_t = cv2.resize(bird_t, (640, 360))
        bird_1 = bird_t.copy()
        rate.sleep()
...
def get_cam2_worker():
    ...
    rospy.loginfo("Birdview vision cam2 ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        temp, bird_t = bird_2_cap.read()  # 2
        bird_t = cv2.resize(bird_t, (640, 360))
        bird_2 = bird_t.copy()
        rate.sleep()
...
def get_cam3_worker():
    ...
    rospy.loginfo("Birdview vision cam3 ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        temp, bird_t = bird_3_cap.read() #3
        bird_t = cv2.resize(bird_t, (640, 360))
        bird_3 = bird_t.copy()
        rate.sleep()
...
def get_cam4_worker():
    ...
    rospy.loginfo("Birdview vision cam4 ready.")
    rate = rospy.Rate(15) # 15hz
    while not rospy.is_shutdown():
        temp, bird_t = bird_4_cap.read() #4
        if not is_HD_Image:
            bird_t = cv2.resize(bird_t, (640, 360))
        bird_4 = bird_t.copy()
        rate.sleep()
...
def topic_birdview_camera():
    ...
    tc1 = threading.Thread(target=get_cam1_worker)
    threads.append(tc1)
    tc2 = threading.Thread(target=get_cam2_worker)
    threads.append(tc2)
    tc3 = threading.Thread(target=get_cam3_worker)
    threads.append(tc3)
    tc4 = threading.Thread(target=get_cam4_worker)
    threads.append(tc4)
    tc1.start()
    tc2.start()
    tc3.start()
    ...
    t1 = threading.Thread(target=pub_birdviewCam1_worker)
    threads.append(t1)
    t2 = threading.Thread(target=pub_birdr5_worker)
    threads.append(t2)
    t3 = threading.Thread(target=pub_birdr10_worker)
    threads.append(t3)
    t4 = threading.Thread(target=pub_birdr20_worker)
    threads.append(t4)
    t5 = threading.Thread(target=pub_birdview_worker)
    threads.append(t5)
    t1.start()
    t2.start()
    t3.start()
    ...
    while not rospy.is_shutdown():
        ...
        """
        '''
        bird_1und = undistort(bird_1,K,D,DIM,debug_imshow)
        bird_2und = undistort(bird_2,K,D,DIM,debug_imshow)
        bird_3und = undistort(bird_3,K,D,DIM,debug_imshow)
        if is_HD_Image:
            bird_4und = undistort(bird_4, K_HD, D_HD, DIM_HD, debug_imshow)
        else:
            bird_4und = undistort(bird_4, K, D, DIM, debug_imshow)
        '''
        #cv2.imshow('bird_2', bird_2)
        rate.sleep()
...
def visual_detector_server():
    ...
    rospy.loginfo("Birdview vision services ready to use.")
