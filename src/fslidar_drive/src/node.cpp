...
int main(int argc, char **argv)
{
    ...
    nh_private.param<std::string>("host", host, "192.168.0.50");
    nh_private.param<int>("port", port, 8487);

    nh_private.param<int>("sample_rate", sample_rate, 20000);
    nh_private.param<int>("rotational_speed", rotational_speed, 17);
    ...
    while (ros::ok())
    {
        ...
        fsld.startDevice(rotational_speed);
        ROS_INFO("done!");
        int new_angle = 0;


        bool is_need_publish = false;
        my_ep.ctrl(EPOLL_CTL_ADD,fsld.socket_fd_,EPOLLIN);
        LaserDataNode tmp_laser;
        while (ros::ok())
        {
            int nread = my_ep.poll(1,500);//超时500ms
            if(nread>0)
            {
                for(int i = 0; i<nread;i++)
                {
                    ...
                    time_cnt_1.record();
                    //std::cout<<" time:"<<time_cnt_1.get_delta_t()<<std::endl;
                    is_need_publish = fsld.unpacking_data();
                    ...
                    for(int i=0;i<ldn_cnt;i++)
                    {
                        ...
                        {
                            std::cout<<"spin once"<<" "<<ldn_tmp->angle<<std::endl;
                            //ROS_INFO("published!");
                        ...
                        {
                            if(ldn_tmp->angle>1440)//旋转90度
                                new_angle = ldn_tmp->angle-1440;
                            else
                                new_angle = 4320+ldn_tmp->angle;
                            (ldn_array + new_angle)->angle = ldn_tmp->angle;
                            (ldn_array + new_angle)->distance = ldn_tmp->distance;
                            last_anl = ldn_tmp->angle;
                        }
                    }
#endif
                    if(is_need_publish==true)
                    {

                        for(int i = 0;i<fsld.laser_scans.size();i++)
                        {
                            tmp_laser = fsld.laser_scans.at(i);
                            if(tmp_laser.angle>1440)//旋转90度
                                new_angle = tmp_laser.angle-1440;
                            else
                                new_angle = 4320+tmp_laser.angle;
                            (ldn_array + new_angle)->angle = tmp_laser.angle;
                            (ldn_array + new_angle)->distance = tmp_laser.distance;
                            last_anl = tmp_laser.angle;
                        }

                       // std::cout<<"spin once"<<" "<<tmp_laser.angle<<std::endl;
                        //ROS_INFO("published!");
                        scan_duration = (ros::Time::now() - start_scan_time).toSec() * 0.001;
                        publish_lidar_data(
                                    pub,
