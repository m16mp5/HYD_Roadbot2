...
public:
    ...
    void startDevice(unsigned int motor_spd);

    void stopDevice();

    void connect(std::string host, int port = 8487);
    void disconnect();
    bool isConnected();
    int socket_fd_;

    std::vector<LaserDataNode> laser_scans;
    std::vector<LaserDataNode> laser_scans_todo;
    bool unpacking_data();
};
