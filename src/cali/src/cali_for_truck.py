...
class MyMainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        rospy.Subscriber("/rcamera/aligned_depth_to_color/camera_info", CameraInfo, self.camera_info_callback)
        rospy.Subscriber("/rcamera/aligned_depth_to_color/image_raw", Image, self.depth_image_callback)
	...
	# self.set_tool1()
        ...
        self.target_pose.A = 180#-90 #self.cur_pose.A
        self.target_pose.B = 0 #self.cur_pose.B
        self.target_pose.C = 180#-180 #self.cur_pose.C
        self.target_pose.S = self.cur_pose.S
        self.target_pose.T = self.cur_pose.T
 
        #p0 = np.array([ 1.200, 0.180,  0.250])
        p0 = np.array([ 1.200, 0,  0])
    ...
    def set_tool(self):
        ...
        if self.use_tool2 == True:
          self.trimmer_default_pose.A =  -180
          self.trimmer_default_pose.B =    0
          self.trimmer_default_pose.C =  180
          self.trimmer_default_pose.X = 1200
          self.trimmer_default_pose.Y =    0
          self.trimmer_default_pose.Z =    0
        else:
